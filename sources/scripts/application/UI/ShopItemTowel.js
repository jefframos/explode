/*jshint undef:false */
var ShopItemTowel = Class.extend({
	...
	build:function(model, isBlock){
		...
		this.equipped = new PIXI.Text('EQUIPPED', {align:'center',font:'40px Vagron', fill:'#db453c', wordWrap:true, wordWrapWidth:500});
		...
		this.valueLabel = new PIXI.Text(this.model.value+'\nPLAYS', {align:'center',font:'50px Vagron', fill:'#ECBC0C', wordWrap:true, wordWrapWidth:500});
		scaleConverter(this.valueLabel.width, this.backShopItem.width, 0.5, this.valueLabel);
		...
		this.equipButton.setPosition(this.backScroll.width/2 - this.equipButton.getContent().width /2,0);//this.backBars.getContent().height - 20 - this.continueButton.height / 2 - 10);
		this.equipButton.clickCallback = this.equipButton.mouseUpCallback = function(){
			if(APP.scrolling){
				return;
			}
			APP.appModel.currentTowelModel = self.model;
			var targetArray = self.screen.towelList;
	...
	updateStats:function(){
		...
		if(!this.pattern){
			...
			this.maskk = new PIXI.Graphics();
			this.maskk.beginFill(this.model.color);
			this.maskk.drawRect(0,0,this.backScroll.width, this.backScroll.height);
			this.maskk.alpha = 1;
			this.container.addChild(this.maskk);


			this.pattern.getContent().mask = this.maskk;
			this.container.addChild(this.pattern.getContent());
			// this.container.setChildIndex(this.pattern.getContent(), 1);
		}
		...
		if(APP.appModel.currentTowelModel.id === this.model.id){
			this.container.addChild(this.equippedBox);
